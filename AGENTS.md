When tagging a blog entry, follow the vocabulary in `docs/tags.md`. Every blog entry gets exactly one first-level domain from the closed set there — `other` if nothing fits, never nothing at all.
